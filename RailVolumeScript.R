...
studyArea <- states(cb = TRUE) %>% filter(STUSPS == "MS") %>% st_transform(proj)

# Download data (SKIP IF ALREADY DOWNLOADED) -----------------------------------------------------------

# Downlaad from: https://hub.arcgis.com/datasets/fedmaps::north-american-rail-lines/about
railLines <- read_sf("https://opendata.arcgis.com/datasets/79685512ba3f4d8a92789781911930b9_0.geojson")
...
gcisCurrent <- read_csv("PublishedCrossingData-05-31-2021.csv", col_types = cols(MilePost = col_double())) %>%
  filter(
...
shpXings <- gcisCurrent[!is.na(gcisCurrent$Latitude),] %>%
  ...
  st_transform(proj) %>% st_intersection(SASF)

# Read in all Accident/Incident data
file_namesAcc <- dir("Tables/FRA", pattern = "^Acc", full.names = TRUE) # List of all files starting with "Acc"
...
railJoin <- st_join(shpXings, railLinesSA, join = st_nearest_feature) %>% st_drop_geometry() %>%
  select(OBJECTID, RROWNER1, trainTot, MaxTtSpd) %>% group_by(OBJECTID) %>% 
  summarize(
...
RailTable %>% write_csv("Tables/Output/RailByOwnerAndType.csv")
...
WarnDev %>% write_csv("Tables/Output/CrossingsByType.csv")
...
tm_basemap(c("CartoDB.Positron", "OpenStreetMap.Mapnik", "Esri.WorldImagery")) +
  ...
  tm_shape(shpXings %>% filter(Type == "PubHwy")) + tm_dots(col = mycols$colSafeOrange, popup.vars = TRUE) +
  tm_shape(SASF) + tm_borders() +
  tm_add_legend(type = "fill", labels = "Public Grade Crossing", col = mycols$colSafeOrange)
...
tm_basemap(c("CartoDB.Positron", "OpenStreetMap.Mapnik", "Esri.WorldImagery")) +
  ...
  tm_shape(xingsCrash) + tm_dots(col = mycols$colSafeOrange, popup.vars = TRUE, size = 0.05) +
  tm_shape(xingsCrash %>% filter(Fatal > 0)) + tm_dots(col = mycols$colSafeRed, size = 0.2) +
  tm_shape(SASF) + tm_borders() +
