...
#### Read/Modify Data ####
bb <- states(cb = TRUE) %>% filter(STUSPS %in% c("WA", "MA", "FL"))

# Read in all Current US crossing data
gcisCurrent <- read_csv("PublishedCrossingData-03-31-2021.csv",
                        col_types = cols(
                          MilePost = col_double())) %>%
  filter(
...
byState %>% summarize(tot = sum(Xings))
...
st_write(shpXings, "shapefiles", "shpXings", driver = "ESRI Shapefile")
...
StateJoin <- full_join(States, byState, by = c("STUSPS" = "StateName")) %>% 
  st_drop_geometry() %>%
  select(
    STUSPS,
    crashes
